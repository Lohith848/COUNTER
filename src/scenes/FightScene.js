...
export default class FightScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Floor area starts around y=440 and ends at y=670. x ranges from 120 to 1160.
    this.physics.world.setBounds(100, 445, 1080, 225);

    // 3. Instantiate Fighters
    // Spawn Akira on the left, Ryuga on the right
    this.player = new Player(this, 300, 560, 'akira_clean');
    this.opponent = new Opponent(this, 980, 560, 'ryuga_clean');

    // 4. Instantiate Adaptive AI
    ...
    this.hud.setRoundText(this.currentRound);

    // 7. Touch Controls Overlay (Mobile / Touch Devices)
  ...
  update(time, delta) {
    ...
    // C. HUD Refresh
    this.hud.update(this.player, this.opponent, this.timeRemaining);
  }
  ...
  startRound() {
    ...
    this.aiBrain.setDifficulty(this.currentRound);
    this.hud.setRoundText(this.currentRound);

    // Double Bell sound
    audio.playBell();

    // Round start countdown text overlays
    const width = this.cameras.main.width;
    const height = this.cameras.main.height;

    const roundTitle = this.add.text(width / 2, height / 2 - 80, `ROUND ${this.currentRound}`, {
    ...
    const announcer = this.add.text(width / 2, height / 2 + 10, 'READY...', {
      fontFamily: '"Press Start 2P"',
      fontSize: '36px',
      color: '#ffaa00',
      stroke: '#000000',
    ...
    // Scale effects
    this.tweens.add({
      targets: [roundTitle, announcer],
      scale: 1.1,
      duration: 1000,
      yoyo: true
    });

    // Timeline for ready-fight sequence
    this.time.delayedCall(1200, () => {
      announcer.setText('FIGHT!');
      announcer.setColor('#00ff88');
  ...
  handleTimeOut() {
    ...
    this.time.delayedCall(1500, () => {
      ...
      if (this.player.health > this.opponent.health) {
        ...
        winnerLabel = 'ROUND WON BY AKIRA';
        this.showFloatyText(width / 2, height / 2, winnerLabel, '#00ff88', 2000, '20px');
      } else if (this.opponent.health > this.player.health) {
        this.roundWins.opponent++;
        winnerLabel = 'ROUND WON BY RYUGA';
        this.showFloatyText(width / 2, height / 2, winnerLabel, '#ff0055', 2000, '20px');
      } else {
        // Tie goes to aggregate landed punches
        if (this.player.stats.landed >= this.opponent.stats.landed) {
          this.roundWins.player++;
          winnerLabel = 'ROUND WON BY AKIRA (DECISION)';
          this.showFloatyText(width / 2, height / 2, winnerLabel, '#00ff88', 2000, '18px');
        } else {
          this.roundWins.opponent++;
          winnerLabel = 'ROUND WON BY RYUGA (DECISION)';
          this.showFloatyText(width / 2, height / 2, winnerLabel, '#ff0055', 2000, '18px');
        }
      }
      ...
      callback: () => {
        ...
        if (count <= 3) {
          ...
          this.time.delayedCall(1500, () => {
            ...
            // Setup Match End Data
            const winner = downedFighter === this.player ? 'Ryuga' : 'Akira';
            this.endMatch(winner, 'KO');
          });
        }
      }
    });
  ...
  checkMatchEnd() {
    // If round 3 finishes, or a fighter has 2 round wins
    if (this.roundWins.player >= 2) {
      this.endMatch('Akira', 'DECISION');
    } else if (this.roundWins.opponent >= 2) {
      this.endMatch('Ryuga', 'DECISION');
    } else if (this.currentRound < 3) {
      // Go to next round
      this.currentRound++;
      this.startRound();
    } else {
      // 3 rounds completed, declare winner based on round points
      const winner = this.roundWins.player >= this.roundWins.opponent ? 'Akira' : 'Ryuga';
      this.endMatch(winner, 'DECISION');
    }
  }
  ...
  endMatch(winner, reason) {
    ...
    // Package stats and transition to ResultScene
    const data = {
      winner: winner,
      reason: reason,
      stats: {
        player: this.player.stats,
        opponent: this.opponent.stats
      }
    };
  ...
  checkPunchHit(attacker, punchType, isExhausted) {
    ...
    // 2. Define range based on punch type
    let range = 140; // Jab
    let damage = 6;
    if (punchType === 'hook') {
      range = 160;
    ...
    // 3. Orientation Check: Attacker must be facing defender to hit
    const isFacing = (attacker.x < defender.x && !attacker.flipX) || 
                      (attacker.x > defender.x && attacker.flipX);

    // 4. Register hit if within range and facing
  ...
  checkClinchHit(attacker) {
    ...
    if (dist <= 125) {
      // Clinch success: push back and stun defender
      defender.unblock(); // Guard break
      defender.stagger(600); // Stun defender
      
      // Push back defender using velocity impulse
  ...
  createHitSpark(x, y, type) {
    ...
    // Draw shockwave ring + spark lines
    ...
    // Tween the spark outwards and fade it
    this.tweens.add({
      targets: spark,
      scaleX: 1.3,
      scaleY: 1.3,
